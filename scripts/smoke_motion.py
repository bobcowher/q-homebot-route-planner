...
and all three eval paths (greedy/softmax/chain) with motion. Runs both the
velocity-only feature (window=1, n+2) and the windowed net-displacement feature
(window=8, n+4) so the wider vector is exercised through every consumer.
...
for window, extra in [(1, 2), (8, 4)]:
    ...
    agent = Agent(env=env, max_buffer_size=5000, goal_layers=2, head_layers=4,
                  use_motion=True, motion_window=window, random_goal_tiles=True)

    assert agent.use_motion and agent.motion_dim == agent.n_actions + extra
    assert agent.q_model.use_motion and agent.q_model.motion_encoder is not None
    assert agent.q_model.motion_window == window
    ...
    *_, motions, next_motions = agent.memory.sample_buffer(4)
    ...
    print(f"window={window}: motion buffer width={motions.shape[1]} OK")
